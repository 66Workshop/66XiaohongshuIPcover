...
### Route A — `SOURCE_PRESERVE_COMPOSITE`

首选。真人姿态基本可用时，把上传真人源作为身份锚点，构图围绕真人适配，尽量减少脸/头重构。

> 这是“最大化保留源人物”的原生 ChatGPT 策略，不是硬像素锁。原生生成式编辑仍可能改写局部像素。

### Route B — `SOURCE_EDIT_MINIMAL`

次选。以真实授权人物图为编辑底图，只改完成构图所需的最小身体区域，脸/头保持接近源图。
...
**原生 ChatGPT 图像生成/编辑没有一个可由本 Skill 调用的“100% 人脸硬锁参数”。**

所以 V0.1 不靠“不能换脸”“100% lock face”做虚假保证，而靠：
...
“任意大幅姿态变化”与“确定性锁定同一张脸”在原生生成式流程里存在真实冲突。`POSE_HIGH` 优先补更接近目标动作的真人授权素材，而不是硬让模型重画脸。

如果未来业务硬要求**源脸像素完全不变**，需要非生成式 compositor/编辑链路兜底；是否值得增加这层复杂度，由 V0.1 Benchmark 决定。
...
完整协议：`prompts/CHATGPT_IDENTITY_LOCK_PROTOCOL.md`。
...
- `SKILL.md` — Identity-Lock 主 Skill
- `docs/INPUT_SPEC.md` — 输入素材标准与角色绑定
- `docs/WORKFLOW.md` — PREFLIGHT → 路由 → 出图 → QA → 回退
...
- identity < 80% 或 C 持续 lookalike → STOP/ROLLBACK 该路线
